...
O presente projeto foi desenvolvido a partir de sockets para dois tipos de aplicações, o primeiro tipo foi desenvolvido utilizando datagram sockets, isto é, sockets que utilizam do protocolo UDP para conectar a aplicação dos medidores com o servidor, já o segundo utilizou de socket e server socket que provém do protocolo TCP para criação de aplicações com a comunicação através da padronização em APIs Rest para troca de dados entre o servidor e o usuário final. Os medidores foram criados para enviar os valores de medição para o servidor, além de prover uma interface simples para alteração dos ritmos de consumo. Com relação ao servidor o mesmo é responsável por armazenar os valores de medição, além do processamento de dados, e finalmente para ser disponível em rede para o usuário consumidor realizar serviços que outrora deveriam ser feitos por funcionários das concessionárias, os serviços desse projeto consistem em: acompanhar o seu consumo de energia, disponibilizar um alerta de consumo para o cliente, gerar e pegar a fatura. Com relação à parte de teste de execução desse protótipo, utilizou-se de contêineres docker para virtualizar e executar a aplicação em um ambiente de rede.
...
<h2>Segue a descrição para a alteração manual:
Passo 1 - Deverá ir nessa classe MI-ConcorrenciaConectividade-Problema-1/src/client/Client.java no método "main" e apenas alterar o IP de 172.16.103.3   para a sua preferência e depois gerar o .jar e a imagem, ou se preferir executar localmente basta alterar o ip para localhost e executar na IDE.
Passo 2 - Deverá ir nessa classe MI-ConcorrenciaConectividade-Problema-1/src/measurer/MeasurerClient.java no método "execMeasurer" e  no metodo "startMeasurer" e alterar o IP de 172.16.103.3 para a sua preferência e depois gerar o .jar e a imagem, ou se preferir executar localmente basta alterar o ip  para localhost e executar na IDE.
Obs: Mas se já quiser testar direto na maquina 3 do larsid basta pegar as imagens que estão no dockerhub através desse link https://hub.docker.com/u/brcz1n ou então executar os docker-compose que estão no projeto na aba stack do portainer
Exemplos de requisição:
  Para o usuario de ID:0 e password:Test1
       - 
       -
       -
       -
